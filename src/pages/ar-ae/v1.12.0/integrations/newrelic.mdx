...
لربط حدث NewRelic مع جلسة المستخدم المسجلة، يجب نشر نص مميز من الواجهة الأمامية إلى الخلفية في كل طلب تريد تتبعه. يمكن القيام بذلك باستخدام رأس HTTP مخصص. في المثال أدناه، نستخدم وظيفة `fetch` لإرسال ذلك الرأس.
...
لأجل أن يربط OpenReplay إدخال سجل NewRelic مع جلسة المستخدم المسجلة، يجب نشر نص مميز كـ `custom_parameter` إلى كل خطأ خلفي يريد تتبعه.
