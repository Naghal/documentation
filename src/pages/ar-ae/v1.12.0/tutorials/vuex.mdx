...
لاحظ اسم متغير البيئة. يبدأ بـ `VUE_APP_` لأنه عندما نفعل ذلك ، سيقوم WebPack باستبدال قيمته في أي مكان نستخدم الكود `process.env.VUE_APP_OPENREPLAY_PROJECT_KEY` لحمايتنا من الحاجة إلى شحن الملف التكوين الفعلي إلى العميل.

الآن ، في مجلد `src` الخاص بك ، أنشئ مجلد `tracker` وملف `index.js` داخله. هذا الملف سيصدر وظيفة واحدة: `startTracker` التي ستكون تكوين كل شيء وبدء المتعقب.
...
كما قمنا بالنظر إليه ، سيتم استبدال سطر `process.env.VUE_APP_OPENREPLAY_PROJECT_KEY` بمفتاح المشروع الفعلي عند تكوين الصفحة.

الآن ، دعنا نضيف الملحق VueX حتى نتمكن من بدء التعقب أيضًا في التغييرات في الحالة.
...
ارجع إلى تعريف هذه الوظيفة ، ولاحظ كيف يوجد لدينا كود لمعالجة الملحقات. في الواقع ، يتم تلقي الملحقات (بمفتاح من اختيارنا) في عنصر `plugins` من كائن التكوين ويتم إرجاع ما نحصل عليه من استدعاء طريقة `use` ، كما نفعل بنفس المفتاح الذي استخدمناه للملحق.

دعني أريك ذلك ، هذه مرة أخرى في ملف `main.js`:
...
1. باستدعاء وظيفة `startTracker` مع خريطة الملحقات. السلسلة "vuexTracker" هي شيء أخرجته في الوقت الذي كنت فيه ، ولكن يمكنك استخدام أي
```jsx
import Vue from 'vue'
