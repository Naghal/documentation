...
إذا كنت ترغب في المتابعة، يمكنك [التحقق من هذا المستودع](https://github.com/deleteman/openreplay-graphql-example) الذي يحتوي على خادم GraphQL وتطبيق المشترك كلاهما.
...
الدالة الأولى ستكون مسؤولة عن تشغيل المسجل وإعداد جميع الملحقات؛ الدالة الثانية ستكون مسؤولة عن استدعاء طريقة "start" فقط.
...
الجزء المثير للاهتمام في الدالة init، هو أنها تعود بكائن مؤلف من كل القيم التي تم إرجاعها من الملحقات. بعض ملحقاتنا ستعود بدالة ستحتاج لاستخدامها لاحقًا (مثل في حالة الملحق GraphQL). يسمح هذا النهج بتهيئة المسجل مع جميع الملحقات في وقت واحد، ومن ثم استخدام القيم المعادة في أي وقت تريد.

## استخدام الملحق

بعد تثبيت الملحق مع npm i @openreplay/tracker-graphql استخدم الكود التالي لاستدعاء الدالة init التي قمنا بتعريفها سابقًا:
...
تعتبر هذه الروابط مثل وسائط خاصة يمكنك استخدامها لتشغيل تدفق البيانات لطلب ما و، في حالتنا، لتسجيله.
...
الكود المذكور أعلاه مأخوذ من وثائق Apollo، في هذه النقطة المسجل والملحق مضبوطون بالفعل، لذلك
```jsx
/**
