...
لأن OpenReplay يجب أن يربط الحدث Sentry مع جلسة المستخدم المسجلة، يجب على الحدث Sentry أن يتم وضع علامة على الرمز الفريد.

### الواجهة الأمامية
إذا كنت تعتمد على Sentry في الواجهة الأمامية الخاصة بك، يمكنك اتباع المثال أدناه.
...
### الخلفية
وإلا، إذا كنت تستخدم SDK Sentry للخلفية، يجب على الرمز الجلسة الانتشار من الواجهة الأمامية في كل طلب تريد تتبعه. يمكن القيام بذلك باستخدام رأس طلب HTTP مخصص. في المثال أدناه، نستخدم API `fetch` لإرسال ذلك الرأس.
...
ثم يمكنك استخراج `openReplaySessionToken` من الرأس وإضافته إلى مجال Sentry الخاص بك (على الأقل باستخدام وسيط أو مزيف).
...
إذا كنت ترغب في ربط عنوان URL لتسجيل OpenReplay مع Sentry وعرضه كعلامة، هنا مثال على كيفية القيام بذلك:
