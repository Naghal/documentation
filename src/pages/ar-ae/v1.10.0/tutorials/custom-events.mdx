...
متتبعنا بشكل افتراضي سيحافظ على العديد من الأشياء المختلفة، بما في ذلك بعض الأخطاء المفيدة، ولكن قد لا يكون هذا كافيًا بالنسبة لك، ولهذا نحن نملك الأحداث المخصصة.

## إضافة الأحداث المخصصة

لهذا المثال، دعنا نأخذ موقع التجارة الإلكترونية العام ونضيف بعض الأحداث لفهم عندما يضيف المستخدم منتجًا إلى سلة التسوق.

بشكل افتراضي، لن يتتبع OpenReplay هذه المعلومات. ومع ذلك، من خلال الأحداث المخصصة، يمكنك بسهولة الاستمرار في التتبع.

لهذا الدرس، سنستخدم مشروع Next.js الذي يتم إعداده بما يتوافق مع نفس البنية كما في الدرس [Next.js](https://docs.openreplay.com/tutorials/next)، لذلك لا تتردد في التحقق من ذلك إذا لم تفعل ذلك بالفعل.

### إنشاء مزود المتتبع

المنطق في هذا الملف يتم شرحه كاملًا في [هذا الدرس](https://docs.openreplay.com/tutorials/next).

كل ما تحتاج إلى معرفته الآن، هو أن هذا هو مزود سيطرة نحن ننشئه للسماح لك بالتفاعل مع المتتبع من خلال العديد من الوظائف.

بالخصوص، سنهتم ب `logIssue` و `logEvent` الذين يتيحان لك إرسال مشكلة مخصصة أو حدث إلى المنصة.

- **الأحداث** مخصصة لتسجيل إجراءات محددة للمستخدم. في حالتنا على سبيل المثال، سنسجل إضافة منتج إلى السلة.
- **المشاكل**، من جهة أخرى، مخصصة لتسجيل الأخطاء التي لا يتم التقاطها تلقائيًا من قبل متتبعنا. في حالتنا، سنقوم بتحديد خطأ شبكة مما يمنعنا من الوصول إلى واجهة برمجة التطبيقات الخارجية. عندما يحدث ذلك، سنسجل مشكلة في المنصة.
...
وتحتوي دوال `logEvent` و `logIssue` على نفس التوقيع، وسنمرر كائنًا محتويًا على خصائص `name` و `data`. سيتم استخدام `name` لتحديد سجلنا في واجهة المستخدم لـ OpenReplay وسيحتوي `data` على المعلومات المسجلة.

**تذكر**: يجب أن تحتوي خاصية `data` على كائن قابل للترجمة.

بعد ذلك، يمكننا تعيين هذا المزود في ملف `_app.tsx` الخاص بنا مثل هذا:
