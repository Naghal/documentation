...
MetaDescription: "كيفية التكامل مع Stackdriver مع OpenReplay ورؤية الأخطاء الخلفية جنبا إلى جنب مع تسجيلات الجلسات."
---

كيفية التكامل مع Stackdriver مع OpenReplay ورؤية الأخطاء الخلفية جنبا إلى جنب مع تسجيلات الجلسات.
...
الصق اسم `Log` و `openreplay-stackdriver` الاعتمادات في لوحة القيادة OpenReplay تحت 'التفضيلات > التكامل'.

![التكامل مع Stackdriver في OpenReplay](/static/stackdriver-1.png#center)

## 3. الانتشار openReplaySessionToken

لربط حدث Stackdriver مع جلسة المستخدم المسجلة، يجب تدفق رمز فريد من نوعه من الواجهة الأمامية إلى الخلفية في كل طلب تريد تتبعه. يمكن القيام بذلك باستخدام رأس HTTP مخصص. في المثال أدناه، نستخدم وظيفة `fetch` لإرسال ذلك الرأس.
...
لأجل أن يربط OpenReplay بإدخال سجل Stackdriver مع جلسة المستخدم المسجلة، يجب تدفق رمز فريد من نوعه كعلامة إلى كل خطأ خلفي يريد تتبعه.

بالأسفل هو مثال في API Python Stackdriver.
