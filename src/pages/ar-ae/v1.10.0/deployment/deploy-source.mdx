...
1. سجل الدخول إلى مستودع الحاويات الخاص بك باستخدام `docker login <registry_url>`. إذا كان لديك حساب في Docker Hub، فببساطة قم بتشغيل `docker login`.
...
إذا كان كل شيء يجري بشكل صحيح، فيجب أن يكون في مستودع Docker الخاص بك قائمة بالصور التي تم بناؤها من الشفرة التي كان لديك، جاهزة للتثبيت.
قائمة الصور هي:
...
1. أنشئ سرية مستودع الحاويات الخاص بك:
...
قم فقط بإضافة قسم لكل صورة لديك في مستودع Docker بالمعلومات التالية:
...
1. افتح `/var/lib/openreplay/vars.yaml` ثم قم بالتحرير:
- `domainName`: هذا هو المكان الذي سيكون متاحًا لـ OpenReplay (على سبيل المث
```bash
