...
💡**ملاحظة لمستخدمي المضيفين نفسهم:** إذا كنت تستخدم نسخة المضيفة الذاتية لـ OpenReplay، فستحتاج أيضًا إلى تحديد خاصية التكوين `ingestPoint`. وتحدد هذه الخاصية نقطة استيراد البيانات للمتعقب. ولا يحتاج المستخدمون الذين يستخدمون السحابة إلى ذلك، لأنه بشكل افتراضي سيعرف المتعقب أين هي نقطة استيراد نسخة الخدمة لهذا، ولكن إذا كنت تستضيفها نفسك، فستحتاج إلى تحديدها (وينبغي أن تكون شيئًا مثل `https://openreplay.mydomain.com/ingest`)
...
بعودته كجزء من تنفيذ دالة التحميل، فإننا يمكن أن نس
```tsx
