...
تثبيت متعقب OpenReplay داخل مشروع مبني على Remix يكون بسيطا بشكل كبير.

بمشروع Remix الخاص بك جاهزا، ابحث عن ملف `root.jsx` داخل مجلد `app`، وضمن الكود الذي يوفره OpenReplay عندما قمت بإنشاء المشروع في المنصة.
...
ولكن لهذا المثال، سننشئ وحدة عينة سنحفظها باسم `tracker.ts` داخل مجلد `app`. وستقوم هذه الوحدة بتصدير دالة تسمى `startTracker` والتي ستقوم بتوليد المتعقب وبدءه.
...
إذا واجهت مشاكل في استيراد وحدة المتعقب من OpenReplay بهذه الطريقة، فتفقد استخدام الاستيراد الديناميكي، مثل هذا:
...
💡**ملاحظة للمستخدمين الذين يستضيفون نفسهم:** إذا كنت تستخدم نسخة المضيفة الذاتية لـ OpenReplay، فستحتاج أيضًا إلى تحديد خاصية التكوين `ingestPoint`. وهذه الخاصية تحدد نقطة استيراج البيانات للمتعقب. لا يحتاج المستخدمون الذين يستخدمون السحابة إلى ذلك، لأنه بشكل افتراضي سيعرف المتعقب أين هي نقطة استيراج نسخة الخدمة لهذا، ولكن إذا كنت تستضيفها نفسك، فستحتاج إلى تحديدها (وينبغي أن تكون شيئًا مثل `https://openreplay.mydomain.com/ingest`)
...
وبعد ذلك سنستخدم العنصر الحاجز `useLoaderData` حتى بعد التحضير يكون للعنصر الرئيسي الوصول إلى البيانات التي تم تحميلها.
...
بعودته كجزء من تنفيذ دالة التحمي
```tsx
