...
i18nReady: true
---
يمكن نشر OpenReplay من المصدر. تحتاج إلى بناء عناصره الرئيسية (الخلفية وواجهة التطبيقات والواجهة الأمامية) ونشرها إلى مستودع الحاويات الخاص بك قبل القيام بالتثبيت.
...
1. سجل الدخول إلى مستودع الحاويات الخاص بك باستخدام `docker login <registry_url>`. إذا كان لديك حساب في Docker Hub، فقم بتشغيل `docker login` بسهولة.

2. قم ببناء العناصر الخلفية ثم قم بنشرها إلى مستودع الحاويات الخاص بك:
...
إذا كان كل شيء يجري بشكل جيد، فيمكنك الحصول على قائمة بالصور التي تم بناؤها من الشفرة التي كان لديك، والتي جاهزة للتثبيت.
...
2. لاستخدام العناصر التي قمت ببنائها ونشرها إلى مستودع الحاويات الخاص بك، قم بتحديث ملف `vars.yaml` الموجود داخل مجلد `openreplay/scripts/helmcharts`.

قم بإضافة قسم لكل صورة لديك في مستودع Docker مع المعلومات التالية:
...
1. افتح ملف `vars.yaml` ثم قم بالتحرير:
- `domainName`: هذا هو المكان الذي سيكون متاحًا لـ OpenReplay (على سبيل المثال openreplay.mycompany.com)
