...
<YoutubeVideo title="مشاهدة كيفية تثبيت OpenReplay في البيئة الخاصة بك في AWS"
  description="إذا لم تكن تحب القراءة، يمكنك اتباع هذا الدرس التعليمي المرئي الذي يوضح لك كيفية تثبيت OpenReplay في AWS"
...
4. اختر نوع النموذج الخاص بك. المواصفات الأدنى هي `2 vCPUs، 8 GB of RAM، 50 GB of storage`، وإلا لن تبدأ خدمات OpenReplay backend بسهولة. لذلك، نوصي ب**على الأقل** `t3.large` (أو ما يعادلها)، وهذا كافي لحجم منخفض / متوسط. إذا كنت تتوقع حركة كبيرة، يجب عليك التزامن من هنا.
5. تكوين تفاصيل النموذج: ضبط إعدادات الشبكة / الشبكة إذا لزم الأمر أو احفظ الإعدادات الافتراضية
6. إضافة التخزين: ضبط الحجم إلى 50 GB (General Purpose SSD (gp2))
7. إضافة العلامات: ضبط الإعدادات أو احفظ الإعدادات الافتراضية
...
OpenReplay يتعامل مع بيانات المستخدم الحساسة وبالتالي يتطلب HTTPS للتشغيل. هذا إلزامي، وإلا لن يبدأ المتعقب بالتسجيل بسهولة. نفس الشيء للمشغل، بدون HTTPS لن تتمكن من إعادة تشغيل جلسات المستخدم.
...
3. أضف مستمع إلى HTTPS (احتفظ فقط بهذا) وتأكد من تحديد نفس الشبكة (الشبكات) فيها يتم تشغيل نموذج OpenReplay.
