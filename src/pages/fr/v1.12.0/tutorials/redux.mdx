...
Notre fichier réducteur (`makeUpReducer.js`) mettra à jour l'état soit avec la liste des produits, soit avec le message d'erreur retourné lorsqu'il y a un problème.
...
Cette logique vous permet d'utiliser le plugin lors de l'initialisation du pisteur et d'accéder et d'utiliser le middleware plus tard.

## Créer le magasin Redux avec le nouveau middleware

Maintenant que le plugin est configuré et que le magasin Redux est correctement créé, tout ce que nous avons à faire maintenant est d'appeler la méthode `start` du pisteur.
