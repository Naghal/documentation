...
- `resourceBaseHref?: string` Se réfère au domaine accessible publiquement où les ressources (styles, polices et icônes) pourraient être récupérées par OpenReplay. Étant donné qu'elles sont nécessaires pour une bonne relecture de session, cette option est utile pour contourner la limitation d'avoir votre site (et donc les ressources) hébergé dans un domaine privé. Exemple: `https://mypublicsite.com/assets/`.
...
- `heatmaps?: boolean` Pour désactiver les cartes de clic. Par défaut: `true`.
...
- `defaultInputMode?: 0 | 1 | 2` Mode de capture par défaut pour les valeurs d'entrée. Respectivement: brut, obscurci ou ignoré. Par défaut: `0` (brut).
...
- `capturePerformance?: boolean` Pour capturer des mét
```js
network?: {
