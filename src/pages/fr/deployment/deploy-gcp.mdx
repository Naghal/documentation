...
La manière la plus simple de gérer SSL dans Google Cloud est de configurer un équilibreur de charge (Google Load Balancing) et de faire fonctionner OpenReplay derrière lui. Une autre option est de générer ou d'utiliser votre propre certificat SSL et de pointer votre sous-domaine (c'est-à-dire openreplay.mycompany.com) vers l'instance OpenReplay. Plus d'informations sur les deux options ci-dessous.
...
> **Note:** Si vous n'avez pas de certificat, générez-en un, qui se renouvelle automatiquement, pour votre sous-domaine (
```yaml
ingress-nginx: &ingress-nginx
  controller:
    config:
