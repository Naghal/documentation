...
1. Tout d'abord, allez à Route 53 (ou votre autre fournisseur de service DNS) et créez un `A Record`. Utilisez le domaine que vous avez fourni précédemment lors de l'étape d'installation et pointez-le vers l'instance à l'aide de son adresse IP publique (peut être trouvé dans le tableau de bord EC2).
