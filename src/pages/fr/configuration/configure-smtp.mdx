...
Ensuite, enregistrez et quittez en utilisant `:wq` pour que les modifications prennent effet. Vous pouvez alors tester la configuration en vous invitant vous-même (en utilisant une autre adresse e-mail) en tant que nouveau membre de l'équipe (dans 'Préférences' > 'Utilisateurs').

## Mailgun

1. Allez à 'Envoi' > 'Domaines' puis cliquez sur 'Ajouter un nouveau domaine'
2. Entrez votre sous-domaine (c'est-à-dire m.mycompany.com) dans 'Nom de domaine' et assurez-vous que 'Créer une autorité DKIM' est cochée, avec de préférence une longueur de clé de 2048
3. Allez chez votre fournisseur de services DNS (des instructions spécifiques sont fournies par Mailgun) et ajoutez **tous les enregistrements affichés**
4. Une fois tous les enregistrements ajoutés, cliquez sur 'Vérifier les paramètres DNS'
5. Allez maintenant à 'Envoi' > 'Paramètres de domaine' > 'Informations d'identification SMTP' et cliquez sur 'Ajouter un nouvel utilisateur SMTP'. Entrez 'Login' (c'est-à-dire openreplay) puis cliquez sur 'Créer des informations d'identification SMTP'. Une fenêtre contextuelle devrait apparaître, cliquez sur 'Copier' pour copier le mot de passe généré.
6. Utilisez les paramètres et informations d'identification SMTP affichés pour configurer SMTP dans OpenReplay. `openreplay -e` et mettre à jour les variables d'environnement ci-dessous dans la section `email`:
```yaml
