...
Chaque enregistrement existe sous la forme d'un fichier et d'une entrée dans la base de données. OpenReplay déverse ce qui est nécessaire pour rejouer une session (mutations DOM, coordonnées de la souris, journaux de la console, activité réseau et bien plus encore) dans 3 fichiers (2 pour la relecture elle-même et 1 pour les données DevTools). Ces fichiers sont par défaut stockés sur votre instance, ils constituent donc la majeure partie de son stockage. Les métadonnées de la session seront stockées dans la base de données PostgreSQL à jamais, mais après 180 jours, le fichier contenant l'enregistrement sera expiré / supprimé via une politique de cycle de vie minio.
...
Ce processus est entièrement automatisé via notre CLI. Exécutez simplement la commande ci-dessous pour nettoyer votre stockage en supprimant des données à la fois de Postgres (où les événements sont stockés) et de minio (où les enregistrements sont enregistrés):
...
Les données peuvent être supprimées à la fois de la base de données (où les événements sont stockés) et de minio (où les enregistrements sont enregistrés).
...
Si vous utilisez minio (installation classique), vous pouvez modifier la politique de cycle de vie par défaut de cette façon:
...
Utilisez la requête SQL ci-dessous si vous souhaitez nettoyer les données de votre base de données (PostgreSQL). Remplacez le `2021-01-01` par la date à partir de laquelle conserver les enregistrements. C'est une suppression en cascade, de sorte que tous les enregistrements ainsi que leurs événements correspondants seront supprimés de la base de données.

<Aside type="caution">
	Ceci est fait à vos risques et périls, alors soyez extrêmement prudent lors de l'exécution de telles actions dans la base de données car elles sont irréversibles.
</Aside>
