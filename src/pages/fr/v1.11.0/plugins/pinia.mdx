...
Ce plugin vous permet de capturer les mutations/états `Pinia` et de les inspecter plus tard lors de la lecture des enregistrements de session. Cela est très utile pour comprendre et résoudre les problèmes.

<Aside type="tip">
Nous avons également un tutoriel détaillé sur la façon d'utiliser le plugin Pinia pour capturer l'état de votre application lors de l'utilisation de cette bibliothèque, vous pouvez le consulter ici ( /tutorials/pinia).
</Aside>
...
Vous pouvez personnaliser le comportement du plugin avec des options pour assainir vos données. Ils sont similaires à ceux du plugin standard `createLogger`.
