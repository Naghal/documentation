...
OpenReplay traite des données d'utilisateur sensibles et nécessite donc HTTPS pour fonctionner. C'est obligatoire, sinon le tracker ne commencerait simplement pas à enregistrer. La même chose s'applique au tableau de bord, sans HTTPS vous ne pourrez pas rejouer les sessions utilisateur.
...
Il est à noter que notre `ingress-ng
```yaml
ingress-nginx: &ingress-nginx
  controller:
    service:
