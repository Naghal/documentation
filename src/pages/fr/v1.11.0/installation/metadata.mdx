...
Si cela n'est pas possible (certaines ou toutes les métadonnées peuvent être définies / connues plus tard dans le flux de navigation, donc bien après le démarrage du tracker), appelez alors la méthode `setMetadata` pour le faire:
