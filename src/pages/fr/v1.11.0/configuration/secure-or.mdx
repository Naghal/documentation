...
Les demandes inter-domaines sont autorisées, par défaut, à partir de toutes les origines (`Access-Control-Allow-Origin: *`). Si vous souhaitez restreindre les enregistrements à quelques domaines seulement, exécutez alors `openreplay -e` et mettez à jour le bloc `http` comme ci-dessous:
