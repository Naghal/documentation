...
Regardez [cette vidéo YouTube](https://youtu.be/t31fj-0t3Dw) pour comprendre comment déployer rapidement OpenReplay sur GCP, ou continuez à lire pour plus de détails.

## Lancez une VM

1. Allez dans le tableau de bord de 'Compute Engine' et sélectionnez 'VM Instances' dans la barre de menu de gauche
2. Cliquez sur 'Créer une instance'
3. Sélectionnez votre région et votre zone préférées
4. Choisissez votre type de machine. Les spécifications minimales sont `2 vCPUs, 8 Go de RAM, 50 Go de stockage`, sinon les services de backend OpenReplay ne démarreront tout simplement pas. Nous vous recommandons donc **au moins** le `n2-standard-2` (ou un équivalent - notez que le `e2-standard-2` a été connu pour donner des problèmes lors du déploiement) qui est suffisant pour un volume faible/modéré. Si vous attendez un trafic élevé, vous devriez le mettre à l'échelle à partir d'ici.
5. Changez votre disque de démarrage pour *Ubuntu 20.04 LTS* ou plus, puis choisissez *SSD persistent disk* pour le type de disque de démarrage et définissez la taille à 50 Go.
...
OpenReplay traite des données d'utilisateur sensibles et nécessite donc HTTPS pour fonctionner. C'est obligatoire, sinon le suivi ne commencera tout simplement pas à enregistrer. Même chose pour le tableau de bord, sans HTTPS vous ne pourrez pas rejouer les sessions d'utilisateur.

La manière la plus simple de gérer SSL dans Google Cloud est de configurer un équilibreur de charge (Google Load Balancing) et d'exécuter OpenReplay derrière lui. Une autre option consiste à générer ou à utiliser votre propre certificat SSL et à pointer votre sous-domaine (c'est-à-dire openreplay.mycompany.com) vers l'instance OpenReplay. Plus d'informations sur les deux options ci-dessous.

### Configurer l'équilibreur de charge Google (option 1)

Première étape consiste à ajouter un groupe d'instances qui sera nécessaire plus tard pour l'équilibreur de charge:
...
5. Sélectionnez *Groupe d'instances* pour 'Type de backend'. Dans 'Backends' > 'Nouveau backend', choisissez le groupe d'instances que vous avez précédemment créé, définissez le port sur `80` puis cliquez sur 'Terminé'.
6. Faites défiler jusqu'à 'Vérification de l'intégrité' et 'Créer une vérification de l'intégrité'. Choisissez HTTP pour le 'Protocole', définissez le 'Port' sur `80` puis modifiez le 'Chemin de la demande' en `/healthz`. Gardez les autres valeurs par défaut, puis cliquez sur 'Enregistrer'.
7. Dans 'Options avancées', cochez 'Activer Cloud CDN', définissez 'Affinité de session' sur `IP client` puis ajoutez `Access-Control-Allow-Origin` dans 'En-têtes de réponse personnalisés'.
8. Cliquez sur 'Créer'
9.  Dans 'Configuration frontend', choisissez HTTPS pour 'Protocole' puis dans 'Certificat' créez un nouveau certificat (géré par Google) ou apportez le vôtre. Cliquez sur 'Terminé'.
10. Aperçu puis cliquez sur 'Créer'
...
1. Tout d'abord, allez dans Cloud DNS (ou votre autre fournisseur de service DNS) et créez un `A Record`. Utilisez le domaine que vous avez fourni précédemment lors de l'étape d'installation et pointez-le vers la VM en utilisant son adresse IP publique (peut être trouvé dans le tableau de bord de Compute Engine).
...
> **Note:** Si vous n'avez pas de certificat, gén
```yaml
ingress-nginx: &ingress-nginx
  controller:
    config:
